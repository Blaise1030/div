...
export function AccordianItem(
  ...
  return div(
    {
      "x-data": `{ id: ${id} }`,
      class: "cursor-pointer group border-b border-border",
    },
    ...children
  );
}

export function AccordianTrigger({}, ...children: HtmlEscapedString[]) {
  return button(
    {
...
export function AccordianContent({}, ...children: HtmlEscapedString[]) {
  return div(
    {"x-show": "isActive(id)", "x-collapse": "", "x-cloak": ""},
    div({class: "p-4 pt-0 opacity-70"}, ...children)
  );
}
